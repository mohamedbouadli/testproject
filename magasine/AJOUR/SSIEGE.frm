VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} SSIEGE 
   Caption         =   "SORTIE SIEGE"
   ClientHeight    =   5175
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   11340
   OleObjectBlob   =   "SSIEGE.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "SSIEGE"
...
If CB1 <> "" And CB1 <> "Selectionner" And CB2 <> "" And CB2 <> "Selectionner" And TB1 <> "" Then    '1
 ...
 If MsgBox("VOULEZ VOUS VRAIMENT AJOUTER CETT SORTIE ?", vbYesNo, "GMCPF") = vbYes Then  '22
    ...
    If Not IsError(Application.Match(CB1, Sheets("SIEGE").Range("b:b"), 0)) Then '2
            ligne = Application.Match(CB1, Sheets("SIEGE").Range("b:b"), 0)
    End If  '2
    If qte > Sheets("SIEGE").Range("d" & ligne).Value Then  '3
    ...
    If CB2.Value = "MEDINA" Then  '4
            
        ligne = 0
        If Not IsError(Application.Match(CB1, Sheets("MEDINA").Range("b:b"), 0)) Then  '5
